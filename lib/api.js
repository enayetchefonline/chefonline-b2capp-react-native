...
// const BASE_URL = 'http://smartrestaurantsolutions.chefonlinetest.co.uk/mobileapi-v2/v2/';
...
export const searchRestaurantsApi = async ({ searchText, orderType, cuisineType, pageNo }) => {
	try {
		...
		const response = await axiosInstance.get(endpoint);
		// result.map((rest) => rest?.rest_id);
		// console.log("searchRestaurant api response:..........", JSON.stringify(response?.data?.app.map((res)=> res?.rest_id)));
		return response?.data;
	} catch (error) {
...
export const verifyDeleteProfileOtp = async ({ user_id, ip_address, otp }) => {
	try {
		const params = { funId: 1445, user_id, ip_address, otp };
		const endpoint = `Tigger.php?${encodeParams(params)}`;
		console.log('🚀 API Request URL for delete profile OTP:', axiosInstance.defaults.baseURL + endpoint);
		const response = await axiosInstance.get(endpoint);
		return response.data;
	} catch (error) {
		console.error('❌ Error in deleteProfileRequest OTP API:', error);
		throw new Error('Failed to delete profile OTP');
	}

}

export const checkUserPhone = async ({ user_id, ip_address, phone_no }) => {
	try {
		// Build query params
		const params = {
			funId: 1446,
			user_id,
			ip_address,
			phone_no, // 👈 include phone number
		};

		const endpoint = `Tigger.php?${encodeParams(params)}`;

		console.log(
			'🚀 API Request URL for checkUserPhone:',
		...
		console.error('❌ Error in checkUserPhone API:', error);
		throw new Error('Failed to add phone number');
	}
};

export const userPhoneVerify= async ({ user_id, ip_address, phone_no, otp }) => {
	try {
		// Build query params
		const params = {
			funId: 1446,
		...
		console.error('❌ Error in checkUserPhone OTP API:', error);
		throw new Error('Failed to verify phone number');
	}
};

//1. https://smartrestaurantsolutions.com/mobileapi-v2/v2/Tigger.php?funId=1447&mobile_no=07731317148
//2. https://smartrestaurantsolutions.chefonlinetest.co.uk/mobileapi-v2/v2/Tigger.php?funId=1447&otp=2447&mobile_no=07845487454

// ✅ Send OTP for Reservation (funId = 1447)
// Example:
//  https://smartrestaurantsolutions.com/mobileapi-v2/v2/Tigger.php?funId=1447&mobile_no=07731317148
export const sendReservationOtp = async ({ mobile_no }) => {
	try {
		const params = {
			funId: 1447,
			mobile_no, // required
		...
		console.log(
			'🚀 API Request URL for sendReservationOtp:',
			axiosInstance.defaults.baseURL + endpoint
		);
		...
		console.error('❌ Error in sendReservationOtp API:', error);
		throw new Error('Failed to send reservation OTP');
	}
};

// ✅ Verify OTP for Reservation (funId = 1447)
// Example:
//  https://smartrestaurantsolutions.chefonlinetest.co.uk/mobileapi-v2/v2/Tigger.php?funId=1447&otp=2447&mobile_no=07845487454
export const verifyReservationOtp = async ({ mobile_no, otp }) => {
	try {
		const params = {
			funId: 1447,
			mobile_no, // required
		...
		console.log(
			'🚀 API Request URL for verifyReservationOtp:',
			axiosInstance.defaults.baseURL + endpoint
		);
		...
		console.error('❌ Error in verifyReservationOtp API:', error);
		throw new Error('Failed to verify reservation OTP');
	}
};

//https://smartrestaurantsolutions.chefonlinetest.co.uk/mobileapi-v2/v2/Tigger.php?funId=44

export const getUkIpVerify = async () => {
	try {
		const params = {
			funId: 44   // required
		};
		...
		console.log(
			'🚀 API Request URL for getIpVerofy:',
			axiosInstance.defaults.baseURL + endpoint
		);
		...
		console.error('❌ Error in getIpVerofy API:', error);
		throw new Error('Failed to getIpVerofy');
	}
};
