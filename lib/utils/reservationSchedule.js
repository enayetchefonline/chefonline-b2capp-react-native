...
/**
 * Generate reservation time slots for a given date (DD/MM/YYYY)
 * based on restaurant schedule.
 *
 * ✅ Uses UK time by treating JS UTC as UK local:
 *    - Today check uses getUTCFullYear/Month/Date
 *    - "Now" minutes uses getUTCHours/getUTCMinutes
 * ✅ Only future slots for *today* (in UK)
 * ✅ 15-minute interval slots between opening & closing
 * ✅ Only uses Reservation shifts: type === '4' or timing_for includes "reservation"
 */
export const getReservationTimeSlots = (scheduleList, dateStr) => {
	...
	// For weekday mapping (local is fine for weekday, we only care about day name)
	...
	const ukMonth = now.getUTCMonth(); // 0–11
