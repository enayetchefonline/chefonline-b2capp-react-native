...
export default function ProfileScreen() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (response?.status === '2') {
        ...
        setDialog({
          visible: true,
          message: response.mess || 'Your account will be deleted soon.',
          isError: false,
        });
      } else {
        setShowDeleteConfirmModal(false);
        setDialog({
          visible: true,
          message: response?.mess || 'Failed to delete profile',
          isError: true,
        });
      }
    } catch (error) {
  ...
  const handleDialogOk = async () => {
    ...
    if (!dialog.isError) {
      // sign out on successful/pending deletion message
      await AsyncStorage.removeItem('accessToken');
      await AsyncStorage.removeItem('userData');
      dispatch(setUser({ user: null, token: null }));
      router.replace('/profile/login');
            ...
            <View style={styles.modalBtnRowPolished}>
              ...
              <TouchableOpacity
                style={[
                  styles.modalBtn,
                  styles.deleteBtn,
                  (!canDelete || deleteLoading) && { opacity: 0.6 },
                ]}
